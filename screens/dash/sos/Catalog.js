...
const Catalog = ({navigation}) => {
  ...
  const [getSubCatid, setSubCatid] = useState('');
  ...
  const callGetCategory = () => {
    ...
    fetch(APILINKS.BASE_URL + APILINKS.GET_CATEGORY_URL, {
      ...
      .then((data) => {
        ...
        setCatid(data.data[0].id);
        callGetSubCat(data.data[0].id);
        // callGetProduct(getid, getToken.msg.token);
      });
  };

  const callGetSubCat = (catId) => {
    setModalVisible(true);
    var form = new FormData();
    form.append('cateid', catId);

    fetch(APILINKS.BASE_URL + APILINKS.GET_SUBCATEGORY_URL, {
      method: 'POST',
      headers: new Headers({
        Accept: 'application/json',
        'Content-Type': 'multipart/form-data',
      ...
      .then((data) => {
        ...
        setSubCatid(data.data[0].id);
        callGetProduct(catId, data.data[0].id);
      });
  };

  const callGetProduct = (catId, subcatId) => {
    setModalVisible(true);
    try {
      var form = new FormData();
      form.append('cateid', catId);
      form.append('subcateid', subcatId);
  ...
  const createProdArray = (dataa) => {
    dataa.forEach((element) => {
      element.product.forEach((element1) => {
        getProductList.push(element1);
      });
    });
  };
      ...
      <View style={{elevation: 1, backgroundColor: COLORS.white, flex: 1}}>
        ...
        <View>
          <FlatList
            ...
            renderItem={({item}) => (
              <TouchableOpacity
                onPress={() => [setCatid(item.id), callGetSubCat(item.id)]}>
                <View
                  style={[
                    getCatid === item.id
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              <TouchableOpacity
                onPress={() => [
                  setSubCatid(item.id),
                  callGetProduct(getCatid, item.id),
                ]}>
                <View>
                  <Text
                    style={[
